...
public class FastCollinearPoints {
    ...
    private void findLineSegments() {
        ...
        for (Point p : pts) {
            ...
            for (int j = 1; j < sortedPts.length; j++) {
                
                double slopeA = p.slopeTo(sortedPts[j - 1]);
                double slopeB = p.slopeTo(sortedPts[j]);
                
                if (slopeA == slopeB) {
                    segLength++;
                    if (j < sortedPts.length - 1) continue;
                }
                
                else {
                    if (segLength > 3) {
                        
                        Point[] line = new Point[segLength];
                        System.arraycopy(sortedPts, start, line, 0, segLength - 1);
                        line[segLength - 1] = p;
                        
                        Arrays.sort(line);
                        
                        if (p.compareTo(line[0]) == 0) {
                ...
                if (segLength > 3) {
                    ...
                    if (line.length == sortedPts.length) {
                        LineSegment ls = new LineSegment(line[0], line[line.length - 1]);
                        segs.add(ls);
                        // numSegs++;
                        break outerloop;
